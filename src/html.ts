export const EXPLORER_HTML_TEMPLATE = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
export function renderExplorerHtml(title: string = DEFAULT_TITLE): string {
  const escaped = title
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#x27;");
  // Use function replacement to avoid $-pattern interpolation in the title.
  // String.prototype.replace treats $&, $`, $', $n as special sequences
  // when the replacement is a plain string — a function avoids this.
  return EXPLORER_HTML_TEMPLATE.replace(/{{TITLE}}/g, () => escaped);
}
